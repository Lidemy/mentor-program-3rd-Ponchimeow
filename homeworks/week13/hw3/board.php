...
if ($resContent->num_rows > 0) {
    ...
    while ($rowMsg = $resContent->fetch_assoc()) {
        ...
        // like
        // 檢查是否已按過
        if (chkLike($conn, $rowMsg['message_id'], $username)) {
            $liked = "-liked";
            $like = "liked";
        } else {
            $liked = "";
...
// 子留言
        ...
        if ($resComment->num_rows > 0) {
            while ($rowComment = $resComment->fetch_assoc()) {
                ...
                // like
                // 檢查是否已按過
                if (chkLike($conn, $rowComment['id'], $username) > 0) {
                    $commentLiked = "-liked";
                    $commentLike = "liked";
                } else {
                    $commentLiked = "";
                ...
                if (isset($username) && $rowComment["member_username"] === $username) {
                    ...
                    echo "<div class='comment-list-header__time__button comment-list-header__time__button-cancel bbutton' data-name='cancel'>取消</div>";
                }
                echo "<div class='comment-list-header__time-date'>$rowComment[time]</div>";
                echo "</div>"; // comment-list-header__info, end div
                echo "</div>"; // comment-list-header, end div
                echo "<p class='comment-text'>";
                echo escape($rowComment['content']);
                echo "</p>";
                echo "</div>"; // comment-list, end div
                echo "</div>"; // comment, end div
            }
        }
        // 子留言留言框
        if (isset($username)) {
        ...
        echo "</div>"; // content, end div
    }
    echo "</div>"; // content-list, end div
}
